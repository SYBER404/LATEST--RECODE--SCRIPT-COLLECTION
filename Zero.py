...
def _____bot________():
	...
	______kom______ = ('Halo Bang NIKI 🤟 SC Lu GG Bang') 
...
def _____LANGSUNG_____():
	...
	______DICKYXD______(' %s[%s+%s] %shasil %zerosOK %sdisimpan ke -> %sok.txt'%(p,p,p,p,p,p,H))

	______DICKYXD______(' %s[%s+%s] %shasil %szerosCP %sdisimpan ke -> %scp.txt'%(p,p,p,p,p,p,K))
...
def ____MANUAL_____():
	...
	if pwek=="":
		...
		______DICKYXD______(' %s[%s+%s] %shasil %szeroOK %sdisimpan ke -> %sok.txt'%(p,p,p,p,p,p,H))

		______DICKYXD______(' %s[%s+%s] %shasil %szeroCP %sdisimpan ke -> %scp.txt'%(p,p,p,p,p,p,K))
...
def apii(uid, pwx):
	...
	for wk in list('\|-/'): 
		sys.stdout.write("\r %s[%s%s%s] [ %sCracking %s] %s/%s zeroOK/%s - CP/%s "%(rgb,K,wk,rgb,rgb,rgb,_______loop______, len(id), len(_______ok_______), len(_______cp_______)))
...
if __name__ == '__main__':
	...
	if sys.version[0]!="2":
		...
		______LuWibu______(" \033[0;97m[\033[0;91m!\033[0;97m] How To Usage : python Zero.py")
